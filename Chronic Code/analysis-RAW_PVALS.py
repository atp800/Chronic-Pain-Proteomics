...
P_THRESHOLD = 0.05
LOG_FC_THRESHOLD = 2.0         # 2.00 extreme (4.0x), 1.58 very high (3.0x), 1.32 very high (2.5x), 1.0 high (2x), 0.58 STANDARD (1.5x), 0.38 relaxed (1.3x), 0.26 very relaxed (1.2x), 0 any change (p-value only)
...
########################################
    ...
    if len(unique_groups_clean) >= 2:
        ...
        # =========================================================
        # DIAGNOSTIC: INSPECT TOP HITS BEFORE FILTERING
        # =========================================================
        print("\n" + "#"*60)
        print("DIAGNOSTICS: TOP 10 LIMMA RESULTS (Unfiltered)")
        print("#"*60)
        
        # 1. Print the top 10 rows. 
        # Since Limma returns sorted by P-value, these are your "best" candidates.
        pd.set_option('display.max_columns', None)
        pd.set_option('display.width', 1000)
        print(df_results[['Protein', 'logFC', 'P.Value', 'adj.P.Val']].head(10))
        pd.reset_option('display.max_columns')
        
        # 2. Check Raw P-value distribution
        raw_sig = df_results[df_results['P.Value'] < P_THRESHOLD].shape[0]
        adj_sig = df_results[df_results['adj.P.Val'] < P_THRESHOLD].shape[0]
        
        print(f"\n--- Significance Summary ---")
        print(f"Proteins with Raw P-Value < {P_THRESHOLD}:      {raw_sig}")
        print(f"Proteins with Adjusted P-Value < {P_THRESHOLD}: {adj_sig}")
        
        if raw_sig > 0 and adj_sig == 0:
            print("\nISSUE: Multiple Testing Correction removing results:")
            print(f"{raw_sig} Proteins that are significant individually (Raw P < {P_THRESHOLD})")
            print("but not surviving FDR correction")
        elif raw_sig == 0:
            print("\n>> DIAGNOSIS: No signal found even in raw p-values.")
        
        print("#"*60 + "\n")
        # =========================================================
        # =========================================================
        # DIAGNOSTIC: CHECK IF RESULTS (FROM RAW PVALS) ARE IMPUTATION ARTIFACTS
        # =========================================================

        print("\n" + "="*60)
        print("DIAGNOSTIC: ARE THESE HITS REAL OR IMPUTATION ARTIFACTS?")
        print("="*60)

        # 1. Get the High FC hits
        # Filter for Raw P < 0.05 and FC > 2 (4x change)
        high_fc_hits = df_results[(df_results['P.Value'] < 0.05) & (abs(df_results['logFC']) > 2.0)].copy()
        
        print(f"Proteins with >4x Change (LogFC > 2): {len(high_fc_hits)}")

        if not high_fc_hits.empty:
            # 2. Check original missing values for these specific proteins
            # We need to look at the data BEFORE imputation. 
            # Note: If you used 'REPLACE_VALS_WITH_DELTAS', this check is harder because the NaNs are gone.
            # But we can check the 'ORIGINAL_DATA_DF' assuming it was loaded before the delta switch if possible,
            # or just check the raw intensities in the final matrix if NaNs were filled.
            
            # Let's look at the correlation between LogFC and Average Expression
            # Artifacts usually have very low Average Expression (because half the data is low noise)
            
            print("\n--- Summary of High FC Hits ---")
            print(high_fc_hits[['logFC', 'AveExpr', 'P.Value']].describe())

            # 3. Quick heuristic: 
            # If a protein was missing in one group, its variance in that group is extremely low (imputed noise).
            # We can try to guess "On/Off" status by looking at the specific top proteins.
            
            print("\n--- Top 10 High FC Proteins ---")
            top_10 = high_fc_hits.head(10)['Protein'].tolist()
            
            # Pull the actual data for these 10 proteins from the matrix used in Limma
            # (Transposed back to rows=samples for easier reading)
            subset_data = X_limma[top_10].copy()
            
            # Add the condition column back temporarily for visualization
            subset_data['Condition'] = ORIGINAL_DATA_DF[CONDITION_COLUMN].values
            
            # Calculate mean by group
            means = subset_data.groupby('Condition').mean()
            print(means.T) 
            
            print("\nINTERPRETATION GUIDE:")
            print("1. If one group is ~0 or very low and the other is high -> It's a 'Presence/Absence' marker.")
            print("   (These are real, but represent binary On/Off states, not regulation).")
            print("2. If both groups are high but different -> It's a massive quantitative change.")
            print("="*60 + "\n")
        #######################################################################
        #######################################################################
        ...
        # Identify Significant Proteins (Using user parameters)
        # Note: changing '>' to '>=' for logFC to catch exactly 0 changes if desired, though unlikely
